...
let hearts          = [];
let audioFade       = null;
let bgm             = null;   // created dynamically on first click
let isMuted         = false;
...
/* ============================================================
   AUDIO — created dynamically inside a user-gesture handler
   so browsers (including GitHub Pages / Safari) allow playback
   ============================================================ */
    ...
    playPromise
      .then(() => {
        ...
        audioFade = setInterval(() => {
          const target = isMuted ? 0 : 0.75;
          if (Math.abs(bgm.volume - target) < 0.03) {
            bgm.volume = target;
            clearInterval(audioFade);
          } else {
            bgm.volume += bgm.volume < target ? 0.025 : -0.025;
          }
        }, 180);
        // Show mute button
      ...
      .catch(() => {
        // Autoplay still blocked — show mute btn as a "tap to play" hint
        muteBtn.textContent = '▶';
        muteBtn.title = 'Tap to play music';
        muteBtn.style.display = 'flex';
        muteBtn.onclick = () => {
          bgm.play().then(() => {
            muteBtn.textContent = '🔊';
...
function toggleMute() {
  isMuted = !isMuted;
  if (bgm) bgm.volume = isMuted ? 0 : 0.75;
...
function beginStory() {
  ...
  // Start music with fade-in (created dynamically for autoplay compliance)
  initAndPlayAudio();
